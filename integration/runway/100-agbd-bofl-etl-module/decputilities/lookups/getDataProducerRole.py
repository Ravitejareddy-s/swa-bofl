...
class Lookup(LookupHandler):
    ...
    def handle(cls, value, context: CfnginContext, provider: BaseProvider, **kwargs) -> str:  # pragma: no cover
        ...
        stack_name = namespace + '-iam-roles'
        stack_output = 'o' + value.lower().replace('-','') + 'rolearn'

        data_producer_role = cls.get_data_producer_role(stack_name, stack_output)
        return data_producer_role
    ...
    @classmethod
    def get_data_producer_role(cls, stack_name, stack_output):
        """finds data producer role arn based on config"""
        try:
